<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
  ...
  <xsl:template name="main">
	...
	<style>	
	  ...
	  /* Categories within features currently just follow the feature */
	...
	<p>The list below shows the categories to which XSD 1.1 tests
	  are assigned, and the number of tests currently in each category.  The
	  figure for a parent category is the sum of the figures for its
	  sub-categories; any tests that are not assigned explicitly to a
	  subcategory are listed in a subcategory called "miscellaneous".</p>
	<p>Categories marked ID involve implementation-defined behavior.</p>
	<ol class="auto-gen">    
  ...
  <xsl:template match="c:test-feature">
    ...
    <xsl:variable name="countMisc" select="count($docRefs[ends-with(@xlink:href, current()/@name)])"/>
    <xsl:variable name="countSubs" select="sum(
        for $c in c:feature-category 
        return count($docRefs[ends-with(@xlink:href, $c/@name)])
      )"/>
    ...
    <li xmlns="http://www.w3.org/1999/xhtml" class="{concat($classname, ' auto-gen')}" id="{@name}">
  ...
  <xsl:template match="c:feature-category">
    <xsl:variable name="ct" 
      select="count($docRefs[ends-with(@xlink:href, current()/@name)])"/>
    <xsl:variable name="classname">
      <xsl:choose>
	<xsl:when test="$ct > 0">
	  <xsl:text>nonzero</xsl:text>
    ...
    <li xmlns="http://www.w3.org/1999/xhtml" 
      ...
      id="{@name}">
      <xsl:value-of select="."/>
      <xsl:text/> (<xsl:value-of select="count($docRefs[ends-with(@xlink:href, current()/@name)])"/>) <xsl:text/>
    </li>
  </xsl:template>
